...
app.get('/getScore', urlencodedParser, function(req, res){
	var sql = 'SELECT wins, losses from score, user where score.user_id = user.id and user.login = "' + login+'"';
	var q = db.query(sql, function(err, rows){
		if(err)
			throw err;
		else
...
app.post('/main/checkSong',urlencodedParser, function(req ,res){
    ...
    if(artist.toUpperCase() == curr_artist.toUpperCase() && song.toUpperCase() == curr_song.toUpperCase())
        {
        res.send("1");
...
app.post('/addPage/addArtist',urlencodedParser, function(req ,res){
    ...
    var sql='INSERT INTO artist(name, description, photo) VALUES("'+artist+'"'+','+'"'+description+'"'+','+'"'+photo+'")';
    var q = db.query(sql, function(err, rows){
        if (err)
            throw err
        else {
...
app.post('/addPage/addAlbum',urlencodedParser, function(req ,res){
	...
	var q = db.query(sql, function(err, rows){
		if (err)
				throw err
		else {artist_id = rows[0].id;
			sql='INSERT INTO album(ganre, album_title, artist_id, cover, released_date) VALUES("'+ganre+'"'+','+'"'+album+'"'+','+'"'+artist_id+'"'+','+'"'+album_cover+'"'+','+'"'+album_year+'")';
...
app.post('/addUser',urlencodedParser, function(req ,res){
	...
	var sql = 'INSERT INTO user(login, password, first_name, last_name, type) VALUES ("'+log+'","'+pass+'","'+fn+'","'+ln+'","guest")';
	console.log(sql);
	var q = db.query(sql, function(err, rows){
						if (err)
						throw err
	...
	var q1 = db.query(sql, function(err, rows){
		if(err)
			throw err;
				...
				var id = rows[0].id;
				sql = 'insert into score(user_id, wins, losses) values ("'+id+'","0","0")';
	...
	var q = db.query(sql, function(err, rows){
		if (err)
		...
		else
				{
						...
						sql = 'INSERT INTO song(title, album_id, artist_id, cover, url) VALUES("'+song+'"'+','+'"'+album_id+'"'+','+'"'+artist_id+'"'+','+'"'+cover+'"'+','+'"'+url+'")';
						var q1 = db.query(sql, function(err, rows){
				if (err)
						throw err
				else {
...
app.post('/editSong',urlencodedParser, function(req ,res){
    var sql = "select song.id, title, name as artist_name, album_title from song, artist, album where song.artist_id = artist.id and song.album_id = album.id";
    var q = db.query(sql, function(err, rows){
        if(err)
            throw err;
        else {
...
app.post('/getAllInf', urlencodedParser, function(req, res){
    var sql = "select song.title as song_title, song.cover as song_cover, song.url as song_audio, artist.name as artist_name, artist.description as artist_description, artist.photo as artist_photo, album.ganre as ganre, album_title, album.description as album_description, album.released_date, song.artist_id, song.album_id from song, artist, album where song.artist_id = artist.id and song.album_id = album.id and song.id = "+ req.body.id;
    var q = db.query(sql, function(err, rows){
        if(err)
            throw err;
        else {
...
app.post('/changeInf', urlencodedParser, function(req, res){
    ...
    var q = db.query(sql, function(err, rows){
        if(err)
            ...
            sql = 'update album set album_title = "'+albumTitle+'", ganre = "'+ganre+'", released_date= '+albumYear+' where id='+ album_id; 
            console.log(sql);
            q = db.query(sql, function(err, rows){
            if(err)
                throw err;
                else{
            sql = "update artist set name = '"+artist_name+"' , description = '"+description+"' , photo= '"+photo+"' where id="+ artist_id; 
            q = db.query(sql, function(err, rows){
            if(err)
                throw err;
                else{
...
app.post('/artistAlbumInfo', urlencodedParser, function(req, res){
    var sql = "select song.cover, album.album_title, album.released_date from song, album where song.artist_id = "+req.body.id+" and song.album_id = album.id";
    var q = db.query(sql, function(err, rows){
        if(err)
            throw err;
        else {
...
app.post('/artistInfo', urlencodedParser, function(req, res){
    var sql = "select name, photo, description from artist where id = "+req.body.id;
    var q = db.query(sql, function(err, rows){
        if(err)
            throw err;
        else {
...
app.post('/getScore', urlencodedParser, function(req, res){
    var sql = "select user.login, score.wins, score.losses from score, user where user.id = score.user_id";
    var q = db.query(sql, function(err, rows){
        if(err)
            throw err;
        else {
